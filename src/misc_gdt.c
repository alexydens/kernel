...
#define GDT_ACCESS_SYSSEG      (1 << 4) /* Is this a system segment? */
#define GDT_ACCESS_NOTSYS      (0 << 4) /* Is this a code/data segment? */
...
/* Flags nibble */
#define GDT_FLAGS_GRANULARITY  (1 << 7) /* Page granularity? */
#define GDT_FLAGS_32BIT        (1 << 5) /* Is this segment in 32 bit mode? */
#define GDT_FLAGS_16BIT        (0 << 5) /* Is this segment in 16 bit mode? */
#define GDT_FLAGS_64BIT        (1 << 4) /* Is this segment in 64 bit mode? */

/* The GDT entry structure */
struct __packed __aligned(0x10) gdt_entry {
  u16 limit_low;
  u16 base_low;
  u8 base_middle;
  u8 access;
...
struct __packed __aligned(0x10) gdtr {
  u16 limit;
  u32 base;
};
